...
const AdminWasteReviewScreen: React.FC = () => {
    ...
    const renderReportItem = ({ item }: { item: WasteReport }) => (
        <View style={styles.card}>
            ...
            <TouchableOpacity onPress={() => { setSelectedReport(item); setModalVisible(true); }}>
                <Text style={styles.viewDetails}>View Details</Text>
            </TouchableOpacity>
        </View>
    );

    if (loading) return <ActivityIndicator size="large" style={{ marginTop: 50 }} color="#1E90FF" />;
                    ...
                    <View style={styles.modalOverlay}>
                        <ScrollView style={styles.modalContent}>
                            ...
                            {selectedReport.status === 'Pending' && (
                                <View style={styles.buttonRow}>
                                    <TouchableOpacity style={styles.approveBtn} onPress={() => updateStatus(selectedReport.id, 'Approved')}>
                                        <Text style={styles.btnText}>Approve</Text>
                                    </TouchableOpacity>
                                    <TouchableOpacity style={styles.rejectBtn} onPress={() => updateStatus(selectedReport.id, 'Rejected')}>
                                        <Text style={styles.btnText}>Reject</Text>
                                    </TouchableOpacity>
                                </View>
                            )}
